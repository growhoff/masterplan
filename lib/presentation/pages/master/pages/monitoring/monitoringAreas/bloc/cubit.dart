...
class CubitMonitoringAreas extends Cubit<StateMonitoringAreas> {
  ...
  CubitMonitoringAreas( this.listAreaMachine): super(StateMonitoringAreas(days: DateTime.now())) {
    int maxChange = getMaxChange(listAreaMachine);
    emit(state.copyWith(listAreaMachine: listAreaMachine, maxChange: maxChange));
  ...
  Future<void> getQuere(List<Map<String, dynamic>>? data) async {
    ...
    await getListMonitorChange(listMonitor);
  ...
  Future<void> getListMonitorChange(List<ItemMachineMonitorMaster> listAreaMonitor) async{
    List<ItemMachineMonitorMaster>? listStatusActiveNew = [];
    List<StatusMachineDTO>? statusActiveList = [];
    for (var element in listAreaMonitor) {
      if (element.machine.shiftSchedule!.count >= state.change){
        List<MonitoringMachine> listStatusNew = [];
        if (element.listStatus.isNotEmpty) {
          listStatusNew = MonitoringTimeList(listTime: MachineChange.getListChange(element.machine)).convertTimeStatusLast(element.listStatus, state.change, element.machine.shiftSchedule!.count);
        } else {
          final quere = await tableMonitoring.selectStatusLastMachineDate(element.machine.id, state.days);
          if (quere != null){
            final lastStatusItem = ConvertDtoModel.converterToMonitorMachine(MonitoringMachineDTO.fromMap(quere));
            listStatusNew = MonitoringTimeList(listTime: MachineChange.getListChange(element.machine)).convertTimeStatusLastItem(lastStatusItem, state.change, state.days, element.machine.shiftSchedule!.count);
          } else {
            listStatusNew = MonitoringTimeList(listTime: MachineChange.getListChange(element.machine)).convertTimeStatusNull(state.change, state.days, element.machine.shiftSchedule!.count);
          }
        }
        final statusActive = getActiveStatus(listStatusNew);
        statusActiveList.add(statusActive);
        listStatusActiveNew.add(ItemMachineMonitorMaster(machine: element.machine, listStatus: listStatusNew, allTime: 0));
      }
    }
      emit(state.copyWith(statusActiveList: statusActiveList, listStatusActiveNew: listStatusActiveNew));
  ...
  void setChange(int change) {
    ...
    getListMonitorChange(state.listMonitor!);
  }
