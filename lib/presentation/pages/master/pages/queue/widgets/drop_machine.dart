import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:master_plan/domain/model/name_index.dart';
import '../bloc/cubit.dart';

class DropMachineQueue extends StatelessWidget {
  const DropMachineQueue(this.activeMachine, this.listItemMachine, {super.key});
  final int activeMachine;
  final List<NameIndex> listItemMachine;
  @override
  Widget build(BuildContext context) {
    return Container(
              decoration: BoxDecoration(
              color: Colors.black12,
              borderRadius: BorderRadius.circular(12),
            ),
              child: DropdownButton<int>(
                isExpanded: true,
                underline: Container(),
                borderRadius: BorderRadius.circular(12),
                hint: const Text('Выберите станок'),
            
                value: activeMachine, 
                items: listItemMachine.map((e) => DropdownMenuItem(value: e.index, child: Text(e.name)),).toList(),
                selectedItemBuilder: (context) => listItemMachine.map((e) => Center(child: Text(e.name),)).toList(),
                onChanged: (value) => value != null ? context.read<CubitQueueMaster>().setActiveMachine(value) : null,
              ),
            );
  }
}