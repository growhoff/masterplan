...
class ChangeOperatorPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider<CubitChangeOperator>(
      create: (context) =>
          CubitChangeOperator(stateMain.machineList, stateMain.machineIdList!),
                      ...
                      builder: (context, state) => Expanded(
                          ...
                          child: ElevatedButton(
                              ...
                              style: ElevatedButton.styleFrom(
                                  backgroundColor: state.change == 1
                                      ? Colors.blue
                                      : Colors.blueGrey,
                                  padding: const EdgeInsets.symmetric(
                                      vertical: 16, horizontal: 5)),
                              child: const Text('1 смена')))),
                  const Spacer(),
                  BlocBuilder<CubitChangeOperator, StateCubitChangeOperator>(
                      builder: (context, state) => Expanded(
                          flex: 5,
                          child: ElevatedButton(
                              onPressed: () => context
                                  ...
                                  .setChange(2),
                              style: ElevatedButton.styleFrom(
                                  backgroundColor: state.change == 2
                                      ? Colors.blue
                                      : Colors.blueGrey,
                                  padding: const EdgeInsets.symmetric(
                                      vertical: 16, horizontal: 5)),
                              child: const Text('2 смена'))))
                ],
              ),
              const SizedBox(height: 8),
              BlocBuilder<CubitChangeOperator, StateCubitChangeOperator>(
