    ...
    for (var machine in machineList!) {
      ...
      for (var dto in listDto) {
        if (dto.machineId == machine.id) {
          if (dto.change!.number == 1) changeOne = ShiftsDistribution(id: dto.id, date: dto.date, change: dto.change!, user: User.fromDTO(dto.user!, null, null), machine: Machine(id: dto.machine!.id,isActivated: dto.machine!.isActivated, inventoryNumber: dto.machine!.inventoryNumber, name: dto.machine!.name, areaId: dto.machine!.areaId));
          if (dto.change!.number == 2) changeTwo = ShiftsDistribution(id: dto.id, date: dto.date, change: dto.change!, user: User.fromDTO(dto.user!, null, null), machine: Machine(id: dto.machine!.id,isActivated: dto.machine!.isActivated, inventoryNumber: dto.machine!.inventoryNumber, name: dto.machine!.name, areaId: dto.machine!.areaId));
        }
      }
