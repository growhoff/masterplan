...
class CubitDistributionDetailsChMas extends Cubit<StateDistributionDetailsChMas> {
  final List<int> areaIdListUser;
  final List<Machine> listMachine;
  final tableOperations = OperatorOperationsTable();
  CubitDistributionDetailsChMas(this.areaIdListUser, this.listMachine) : super(const StateDistributionDetailsChMas()){
      tableOperations.table.stream(primaryKey: ['id']).inFilter('area_id', [areaIdListUser]).listen((event){
      }).onData((data)async {
          ...
          for (var order in elSearch.list) {
            ...
            for (var item in iModBatch.list) {
              if (item.operationId == order) {
                if ((item.statusId == 2 || item.statusId == 4) ) {listReady.add(item);break;}//убрал проверку && item.areaId == areaIdUser
