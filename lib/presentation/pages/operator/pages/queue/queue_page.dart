                  ...
                  child: Padding(
                    ...
                    child:  Column(
                          children: [
                            ...
                            const RowList(mod: null, text1: 'Деталь', text2: 'Операция', text3: 'T факт, мин', text4: '№ опт. парт.', color: Colors.white),
                            const Divider(),
                            const SizedBox(height: 8),
                            const Text('Обрабатывается'),
                            const SizedBox(height: 8),
                            const Divider(),
                            RowList(
                              ...
                              text4: dataPage.operActive != null ? '${dataPage.operActive!.list.length}' : 'none',
                            ),
                            const Divider(),
                            const SizedBox(height: 8),
                            const Text('В очереди'),
                            const SizedBox(height: 8),
                            const Divider(),
                            dataPage.operQueueList == [] 
                              ? const Text('Список пуст')
                              : Column(
                                children: dataPage.operQueueList.map((item) => RowList(
                                    color: Colors.grey,
                                    ...
                                    // text3: '${ item.list.first.timeplan}',
                                    text3: '-',
                                    text4: '${ item.list.length}',
                                    )).toList(),
                              ),
                            ...
                            dataPage.operReadyList == [] 
                              ...
                              : Column(
                                children: dataPage.operReadyList.map((item) => RowList(
                                    ...
                                    text1: '${ item.list.first.batch.numberRS} ${ item.list.first.batch.name}',
                                    text2: '${ item.list.first.operation.number} ${ item.list.first.operation.name}',
                                    text3: TimeConverter().convertTimeFromSecondsToMin(item.list.first.timeworking ?? 0),
                                    text4: '${ item.list.length}',
                                    )).toList(),
                              ),
                            const Divider(),
                            const SizedBox(height: 8),
                            const Text('Отбраковано'),
                            const SizedBox(height: 8),
                            const Divider(),
                            dataPage.operReadyList == [] 
                              ? const Text('Список пуст')
