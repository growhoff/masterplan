...
class MonitoringMachineTable extends SupabaseTable {
  ...
  static const userMonitor =
      '*, z_status_machine(*), z_staff($userStaff), z_machine(*, z_shift_schedule(*)), z_batch(*)';
  ...
  Future<List<Map<String, dynamic>>> selectByAreasIdsList(
      {required List<int> areasIdsList}) async {
    var res = await table
        .select('*, z_batch(*),z_machine!inner(*, z_area(*,z_unit(*))), z_status_machine(*),  z_staff(*)')
        .inFilter('z_machine.area_id', areasIdsList);
    return res;
  }
  ...
  Future<List<Map<String, dynamic>>> selectListIdMachineChange(
      List<int> listIdMachine, int change, String date, int staffId) {
    return table
        .select(userMonitor)
        .inFilter('machine_id', listIdMachine)
        .eq('change_id', change)
        .eq('date', date)
        .eq('staff_id', staffId)
  ...
  Future<List<Map<String, dynamic>>> selectListIdMachineChangeLastDay(
      List<int> listIdMachine) {
    return table
        .select(userMonitor)
        .inFilter('machine_id', listIdMachine)
        .eq('time_stop', 0)
  ...
  Future<List<Map<String, dynamic>>> selectStatus(
      String date, int change, int idMachine) {
    return table
        .select('*')
        .eq('date', date)
        .eq('status_machine_id', 8)
        .eq('change_id', change)
  ...
  Future<Map<String, dynamic>?> selectStatusLastMachineDate(
      int idMachine, DateTime date) async {
    final queue = await table
        .select(userMonitor)
        .eq('machine_id', idMachine)
        .lte('date', date)
  ...
  Future<List<Map<String, dynamic>>> selectIdMonitor(
      int machineId, int batchId, int optPathOper) {
    return table
        .select(userMonitor)
        .eq('operation_id', optPathOper)
        .eq('batch_id', batchId)
  ...
  Future<List<Map<String, dynamic>>> selectListIdMachine(
      List<int> listId, DateTime date) {
    String filters = '';
    for (var i = 0; i < listId.length; i++) {
      if (i == (listId.length - 1)) {
        filters += 'machine_id.eq.${listId[i]}';
  ...
  Future<List<Map<String, dynamic>>> selectListId(
      List<int> listId, DateTime date) {
  ...
  Future<void> updateId(int id, int timeStop, int timeWork) async {
    return table
        .update({'time_stop': timeStop, 'time_working': timeWork}).eq('id', id);
  }

  Future<void> updateIdComment(
      int id, int time, String comment, int timeWork) async {
    return table.update({
      'time_stop': time,
      'comment': comment,
      'time_working': timeWork
    }).eq('id', id);
  }
